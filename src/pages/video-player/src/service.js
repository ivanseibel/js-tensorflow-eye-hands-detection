...
export default class Service {
  ...
  async handleBlinked(img) {
    const predictions = await this.#estimateFaces(img)
    ...
    for (const prediction of predictions) {
      ...
      // True if the right eye is closed
      const blinked = (rightEAR <= EAR_THRESHOLD) && !(leftEAR <= EAR_THRESHOLD)
